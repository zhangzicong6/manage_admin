                    ...
                    if (message.MsgType === 'text') {
                        ...
                        if (message.Event === 'subscribe') {
                            ...
                            if (message.EventKey == 'KEY_GERENZHONGXIN') {
                                res.reply('好开心，终于等到你的关注 ~\r\n\r\n你往后淘寳可别再花冤枉钱了，下单之前快来查券拿奖金！\r\n\r\n淘寳购物如何拿奖金？\r\n\r\n①☞ 图文教程：<a href="https://w.url.cn/s/ALbRRgu">点击查看</a>\r\n\r\n②☞ 视频教程：<a href="https://w.url.cn/s/ASjqD5J">点击查看</a>\r\n\r\n一一一🍒使用教程简介一一一 \r\n<搜索优惠>：搜索+商品名称 \r\n<指定商品优惠查询>：请将淘宝商品分享给我！\r\n\r\n一一一一🍒常用指令一一一一\r\n账户信息请回复：个人信息\r\n邀请好友请回复：邀请有礼\r\n订单查询请回复：订单\r\n余额提现请回复：提现')
            ...
            if (auction != 10000) {
                async.waterfall([
                    function (callback) {
                        ...
                        user.save();
                        var conf = await mem.get("configure_"+user.code);
                        if(!conf){
                            conf = await ConfigModel.find({code:user.code})
                            conf = conf[0]
                            await mem.set("configure_"+user.code,conf,30*24*3600)
                        }
...
async function bind_user(openid, code, ticket, res) {
    ...
    let conf = await mem.get("configure_"+code);
    if(!conf){
        conf = await ConfigModel.find({code:code})
        conf = conf[0]
        await mem.set("configure_"+code,conf,30*24*3600)
    }
